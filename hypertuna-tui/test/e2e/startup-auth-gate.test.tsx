...
const BASE_OPTIONS: RuntimeOptions = {
  cwd: process.cwd(),
  storageDir: '/tmp/hypertuna-tui-e2e-startup-gate',
  noAnimations: true,
  logLevel: 'info'
}
...
describe.sequential('TUI e2e startup authentication gate', () => {
  it('shows setup auth choice when no stored accounts exist', async () => {
    const controller = new MockController(BASE_OPTIONS)
  ...
  it('completes no-account generated flow through keys, profile, discovery, and bootstrap', async () => {
    ...
    try {
      ...
      await waitFor(() => frame(instance).includes('Generated Account Keys'))
